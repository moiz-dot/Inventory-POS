...
<template>
    <AppLayout title="POS Terminal">
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-xl font-bold text-primary tracking-tight">Edge POS Terminal Cashier</h2>
                    </div>
                    <p class="text-xs text-muted-light font-mono mt-0.5">Local offline-capable checkout engine</p>
                </div>
            ...
            <!-- Success Modal -->
            <div v-if="posSuccess" class="fixed inset-0 z-50 bg-modal backdrop-blur-sm flex items-center justify-center p-4">
                <div class="bg-primary border border-tertiary rounded-3xl p-6 max-w-sm w-full space-y-6 text-center animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div>
                        <h3 class="text-base font-bold text-primary">Consolidated Ledger Committed</h3>
                        <p class="text-xs text-muted mt-1 flex items-center justify-center gap-1">
                            <component :is="Receipt" class="w-3 h-3" />
                            Transaction ID: <span class="font-mono text-primary">{{ posSuccess }}</span>
                        </p>
                    </div>
                    <button @click="posSuccess = null" class="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-xs text-primary font-semibold rounded-lg transition-colors flex items-center justify-center gap-2">
            ...
            <div class="grid lg:grid-cols-12 gap-8 items-start">
                ...
                <div class="lg:col-span-7 space-y-4">
                    ...
                    <form @submit="handleBarcode" class="relative">
                        ...
                        <input 
                            ...
                            class="w-full bg-card-hover border border-card focus:border-blue-500/40 focus:ring-1 focus:ring-blue-500/20 rounded-xl py-2.5 pl-12 pr-4 text-xs text-secondary outline-none transition-all font-mono placeholder:text-muted-light" 
                        />
                    </form>
                    ...
                    <div class="grid sm:grid-cols-3 gap-4">
                        <div 
                            ...
                            :class="p.stock <= 0 ? 'border-tertiary-50 bg-secondary-10 opacity-40 cursor-not-allowed' : 'border-card bg-card-light hover:border-blue-500/40 hover:bg-card-hover'"
                        >
                            <div class="relative">
                                <div class="flex items-start justify-between">
                                    <span class="text-[9px] font-mono text-muted-light block flex items-center gap-1">
                                ...
                                <h4 class="text-xs font-semibold text-primary mt-1 leading-snug group-hover:text-blue-200 transition-colors">{{ p.name }}</h4>
                            </div>
                            <div class="flex justify-between items-center border-t border-light pt-3 mt-3">
                                <span class="text-xs font-bold text-primary font-mono flex items-center gap-0.5">
                            ...
                            <div v-if="p.stock <= 0" class="absolute inset-0 bg-input backdrop-blur-sm flex items-center justify-center">
                ...
                <!-- Right Column - Cart -->
                <div class="lg:col-span-5 bg-card-hover border border-card p-6 rounded-2xl space-y-6 sticky top-6">
                    <!-- Cart Header -->
                    <div class="flex justify-between items-center border-b border-light pb-4">
                        <div class="flex items-center gap-2">
                            <component :is="ShoppingCart" class="w-4 h-4 text-blue-400" />
                            <h3 class="text-xs font-semibold text-muted font-mono uppercase">Checkout Roster</h3>
                            <span class="text-[8px] font-mono bg-secondary text-muted px-1.5 py-0.5 rounded-full">{{ cart.length }}</span>
                        </div>
                        <button @click="cart = []" class="text-placeholder hover:text-red-400 text-[10px] font-mono flex items-center gap-1 transition-colors">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="flex gap-2">
                            <select 
                                v-model="custId" 
                                class="flex-1 bg-primary border border-card focus:border-blue-500/40 rounded-lg p-2.5 text-xs text-secondary outline-none transition-all"
                            >
                    ...
                    <div class="space-y-2 max-h-[220px] overflow-y-auto pr-1 custom-scroll">
                        <div v-if="cart.length === 0" class="text-center py-8 text-muted-light text-xs font-mono flex flex-col items-center gap-2">
                            <component :is="ShoppingCart" class="w-8 h-8 text-placeholder" />
                            Shopping Cart Empty
                        </div>
                        <div v-for="(item, idx) in cart" :key="idx" class="group bg-input p-3 rounded-xl border border-light hover:border-card transition-all flex justify-between items-center text-xs">
                            <div>
                                <h5 class="font-semibold text-primary group-hover:text-blue-200 transition-colors">{{ item.product.name }}</h5>
                                <span class="text-[10px] font-mono text-muted-light flex items-center gap-1">
                            ...
                            <div class="flex items-center gap-3">
                                <span class="font-mono font-bold text-primary">${{ (item.product.salePrice * item.quantity).toLocaleString() }}</span>
                                <button @click="cart.splice(idx, 1)" class="text-placeholder hover:text-red-400 transition-colors p-1 rounded hover:bg-red-500/10">
                    ...
                    <!-- Totals -->
                    <div class="border-t border-light pt-4 space-y-1 text-xs font-mono">
                        ...
                        <div class="flex justify-between text-primary font-bold text-sm border-t border-dashed border-tertiary pt-2 mt-1">
                            <span class="flex items-center gap-1">
                    ...
                    <button 
                        ...
                        class="w-full py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-xs text-primary font-bold rounded-lg shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2"
                    >
