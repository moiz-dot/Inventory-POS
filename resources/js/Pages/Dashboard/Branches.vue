...
<template>
    <AppLayout title="Retail Branches">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                <div class="flex items-center gap-2">
                    ...
                    <h2 class="text-xl font-bold text-white tracking-tight">Active Roster Branches</h2>
                    <span class="text-[9px] font-mono bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded-full">{{ branches.length }} active</span>
                </div>
                <p class="text-xs text-slate-500 font-mono mt-0.5">Multi-branch ledger integration status</p>
            </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="br in branches" :key="br.id" class="group bg-slate-900/40 border border-slate-800/80 p-6 rounded-2xl relative hover:border-slate-700/80 hover:bg-slate-900/60 transition-all">
                <!-- Status Dot -->
                ...
                <div class="flex items-center gap-1.5 mb-1">
                    <component :is="MapPin" class="w-3 h-3 text-slate-500" />
                    <span class="text-[9px] font-mono text-slate-500 uppercase tracking-widest">{{ br.location }}</span>
                </div>

                <!-- Branch Name -->
                <h3 class="text-sm font-bold text-white mt-1.5 flex items-center gap-2">
                    <component :is="Store" class="w-3.5 h-3.5 text-blue-400 flex-shrink-0" />
                    {{ br.name }}
                </h3>

                <!-- Manager -->
                <p class="text-[10px] font-mono text-slate-400 mt-0.5 flex items-center gap-1.5">
                    <component :is="User" class="w-3 h-3 text-slate-500" />
                    Director: {{ br.manager }}
                </p>

                <!-- Stats -->
                <div class="border-t border-slate-800/60 pt-4 mt-4 grid grid-cols-2 gap-2 text-xs font-mono">
                    <div>
                        <span class="text-[10px] text-slate-500 block flex items-center gap-1">
                            <component :is="DollarSign" class="w-3 h-3" />
                            Life Sales
                        </span>
                        <span class="text-white font-bold">${{ br.sales.toLocaleString() }}</span>
                    </div>
                    <div>
                        <span class="text-[10px] text-slate-500 block flex items-center gap-1">
                        ...
                        <span class="text-slate-300 font-bold">{{ br.stockCount.toLocaleString() }}</span>
                    </div>
                </div>
        ...
        <!-- Quick Stats Footer -->
        <div class="mt-8 bg-slate-900/20 border border-slate-800/80 rounded-2xl p-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div class="text-center">
                <div class="flex items-center justify-center gap-1.5">
                    <component :is="Store" class="w-4 h-4 text-blue-400" />
                    <span class="text-lg font-bold text-white">{{ branches.length }}</span>
                </div>
                <p class="text-[9px] font-mono text-slate-500 uppercase">Total Branches</p>
            </div>
            <div class="text-center">
                <div class="flex items-center justify-center gap-1.5">
                    <component :is="CheckCircle" class="w-4 h-4 text-emerald-400" />
                    <span class="text-lg font-bold text-white">{{ branches.filter(b => b.status === 'active').length }}</span>
                </div>
                <p class="text-[9px] font-mono text-slate-500 uppercase">Active</p>
            </div>
            <div class="text-center">
                <div class="flex items-center justify-center gap-1.5">
                    <component :is="DollarSign" class="w-4 h-4 text-emerald-400" />
                    <span class="text-lg font-bold text-white">${{ branches.reduce((sum, b) => sum + b.sales, 0).toLocaleString() }}</span>
                </div>
            ...
            <div class="text-center">
                <div class="flex items-center justify-center gap-1.5">
                    ...
                    <span class="text-lg font-bold text-white">{{ branches.reduce((sum, b) => sum + b.stockCount, 0).toLocaleString() }}</span>
                </div>
                <p class="text-[9px] font-mono text-slate-500 uppercase">Total Stock</p>
            </div>
        </div>
    </AppLayout>
</template>